...
module constants_mcp
    ...
    integer, parameter :: L=250	  !Linear dimension
    integer, parameter :: nrepet=100 !nombre de répétition
    integer, parameter :: N=L*L ! surface 
    real, dimension(L**2) :: ns=0.0 ! nombre de cluster de taille s
    ...
    integer, dimension(N) :: ptr, order   ! order va generer l'ordre par lequel les points vont être placé
...
subroutine convolution
	...
	open(222,file='out250.dat',status='replace',action='write')
...
subroutine PBCpercolate(m)
	...
	rlc=empty		! rlc will contain the position of the root of the largest non-percolating cluster
		...
		mp=mp+1
    			if(mp==0 .and. m==1) then
    				compt=1
				do while(compt<=imax)
					ii=int(rand(0)*(799-301))+301
					jj=int(rand(0)*(799-301))+301
					ss=dwhere(ii,jj)
					rr=findroot(ss)
	...
	integer, dimension(L**2) :: wrapping
		...
		do j=jj-rmax,jj+rmax
			...
			rayon=ceiling (sqrt(d))
            		if(wrapping(r)==1 .and. rayon<=rmax .and. rayon>0) then
...
program main
    ...
    ns=ns/nrepet
    do m=1, N
    	write(14,*) m,perc_prob_n(m),p_infinite_n(m),lcn(m),susceptibility(m),ns(m)
    enddo
    do m=1, N
    	write(18,*) m,ns(m)
    enddo
